...
template <class T>
class UnDirectedGraph: IGraph<T>
{
private:
...
public:
    ...
    UnDirectedGraph(const UnDirectedGraph& other) : vertices()
    {
    ...
    void AddEdge(int from_id, int to_id, T weight = 0) override
    {
        vertices.Get(from_id)->AddEdge(to_id, weight);
        vertices.Get(to_id)->AddEdge(from_id, weight);
    }
